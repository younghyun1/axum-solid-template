// autogenerated metadata - do not edit!
#[derive(Debug)]
pub struct LibVersion {
    pub name: &'static str,
    pub version: &'static str,
}

impl LibVersion {
    pub fn get_name(&self) -> &'static str {
        self.name
    }
    pub fn get_version(&self) -> &'static str {
        self.version
    }
}

pub struct LibVersionMap {
    pub list: &'static [LibVersion],
}

impl LibVersionMap {
    pub fn get(&self, name: &str) -> Option<&LibVersion> {
        self.list.iter().find(|v| v.get_name() == name)
    }
}

/// AUTO-GENERATED BY build.rs
pub const PROJECT_NAME: &str = "be";
pub const PROJECT_VERSION: &str = "0.1.0";
pub const BUILD_TIME_UTC: &str = "2026-05-17T19:52:18.681005525+00:00";
pub const RUSTC_VERSION: &str = "rustc 1.97.0-nightly (d3cd04068 2026-05-16)";
pub const LIB_VERSIONS: [LibVersion; 40] = [
    LibVersion {
        name: "ammonia",
        version: "4.1.2",
    },
    LibVersion {
        name: "argon2",
        version: "0.5.3",
    },
    LibVersion {
        name: "axum",
        version: "0.8.9",
    },
    LibVersion {
        name: "axum-extra",
        version: "0.12.6",
    },
    LibVersion {
        name: "axum-server",
        version: "0.8.0",
    },
    LibVersion {
        name: "bytes",
        version: "1.11.1",
    },
    LibVersion {
        name: "chrono",
        version: "0.4.44",
    },
    LibVersion {
        name: "comrak",
        version: "0.52.0",
    },
    LibVersion {
        name: "diesel",
        version: "2.3.9",
    },
    LibVersion {
        name: "diesel-async",
        version: "0.9.0",
    },
    LibVersion {
        name: "diesel_migrations",
        version: "2.3.2",
    },
    LibVersion {
        name: "dotenvy",
        version: "0.15.7",
    },
    LibVersion {
        name: "email_address",
        version: "0.2.9",
    },
    LibVersion {
        name: "jsonwebtoken",
        version: "10.4.0",
    },
    LibVersion {
        name: "lettre",
        version: "0.11.22",
    },
    LibVersion {
        name: "mimalloc",
        version: "0.1.50",
    },
    LibVersion {
        name: "mime_guess",
        version: "2.0.5",
    },
    LibVersion {
        name: "moka",
        version: "0.12.15",
    },
    LibVersion {
        name: "nutype",
        version: "0.7.0",
    },
    LibVersion {
        name: "rand",
        version: "0.8.6",
    },
    LibVersion {
        name: "rand",
        version: "0.9.4",
    },
    LibVersion {
        name: "rand",
        version: "0.10.1",
    },
    LibVersion {
        name: "rust-embed",
        version: "8.11.0",
    },
    LibVersion {
        name: "rustls",
        version: "0.23.40",
    },
    LibVersion {
        name: "scc",
        version: "3.7.1",
    },
    LibVersion {
        name: "serde",
        version: "1.0.228",
    },
    LibVersion {
        name: "serde_json",
        version: "1.0.149",
    },
    LibVersion {
        name: "sha2",
        version: "0.10.9",
    },
    LibVersion {
        name: "sha2",
        version: "0.11.0",
    },
    LibVersion {
        name: "tantivy",
        version: "0.26.1",
    },
    LibVersion {
        name: "tokio",
        version: "1.52.3",
    },
    LibVersion {
        name: "tower-http",
        version: "0.6.10",
    },
    LibVersion {
        name: "tower_governor",
        version: "0.8.0",
    },
    LibVersion {
        name: "tracing",
        version: "0.1.44",
    },
    LibVersion {
        name: "tracing-appender",
        version: "0.2.5",
    },
    LibVersion {
        name: "tracing-subscriber",
        version: "0.3.23",
    },
    LibVersion {
        name: "utoipa",
        version: "5.5.0",
    },
    LibVersion {
        name: "utoipa-swagger-ui",
        version: "9.0.2",
    },
    LibVersion {
        name: "uuid",
        version: "1.23.1",
    },
    LibVersion {
        name: "zeroize",
        version: "1.8.2",
    },
];
pub const LIB_VERSION_MAP: LibVersionMap = LibVersionMap {
    list: &LIB_VERSIONS,
};
